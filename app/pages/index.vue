...
useHead({
  ...
  noscript: [
    { innerHTML: "<style>.hero-cloak{visibility:visible!important}</style>" },
  ],
});
...
const { data: worksData } = await useAsyncData<Work[]>(
  "works",
  async () => {
...
onMounted(() => {
  ...
  heroCtx = gsap.context(() => {
    ...
    heroSplit = SplitText.create(title, {
      type: "chars",
    ...
    gsap
      .timeline({
        defaults: { ease: "ease01" },
        onComplete: () =>
          title
            .querySelector("[data-hero-hatch]")
            ?.classList.add("hero-sketch"),
      })
      .from(
        el.querySelector("[data-hero-eyebrow]"),
        { opacity: 0, x: -20, duration: 0.5, ease: "ease02" },
        0,
      )
      .from(
        heroSplit.chars,
        { yPercent: 100, opacity: 0, stagger: 0.02, duration: 0.7 },
        0.1,
      )
      .from(
        el.querySelector("[data-hero-img]"),
        { opacity: 0, scale: 0.9, duration: 0.9 },
        0.15,
      )
      .from(
        el.querySelectorAll("[data-hero-deco]"),
        { opacity: 0, scale: 0.4, stagger: 0.06, duration: 0.5 },
        0.5,
      )
      .from(
        el.querySelectorAll("[data-hero-item]"),
        { opacity: 0, y: 20, stagger: 0.08, duration: 0.55, ease: "ease02" },
        0.55,
      )
      .fromTo(
        el.querySelector("[data-hero-accent]"),
        { opacity: 0 },
        {
          opacity: 1,
          duration: 0.4,
          ease: "none",
          keyframes: { opacity: [0, 1, 0, 1, 0.4, 1], easeEach: "steps(1)" },
        },
        0.7,
      );
  }, el);
...
<template>
  ...
  <section ref="hero" class="hero-cloak relative overflow-hidden">
    ...
    <div aria-hidden="true" class="pointer-events-none absolute inset-0">
      ...
      <div
        class="absolute inset-y-0 right-[14%] w-px bg-rose-200/40 hidden md:block"
      ></div>
    </div>

    <div
      class="relative max-w-6xl mx-auto px-6 md:px-10 grid md:grid-cols-12 gap-8 md:gap-6 items-center min-h-[80vh] pt-16 pb-12"
    >
      <!-- text -->
      <div class="md:col-span-7 order-2 md:order-1">
        ...
        <div data-hero-eyebrow class="flex items-center gap-3 mb-6">
          ...
          <span
            class="font-mono text-[11px] tracking-[0.35em] uppercase text-rose-400"
            >Hey 👋🏻 I'm fuki</span
          >
        </div>
        ...
        <h1
          data-hero-title
          class="font-black tracking-tight leading-[0.92] text-[clamp(2.5rem,7vw,5.5rem)] text-neutral-900"
        >
          <span class="block">meccha</span>
          <span class="block">tsuyotsuyo</span>
          <span data-hero-hatch class="hero-line ink-rose block"
            >Illustrator</span
          >
        </h1>
        ...
        <div
          data-hero-item
          class="mt-7 h-px max-w-md bg-linear-to-r from-rose-300 via-rose-200 to-transparent"
        ></div>
      ...
      <div
        class="md:col-span-5 order-1 md:order-2 flex justify-center md:justify-end"
      >
        <div class="relative w-56 h-56 md:w-72 md:h-72">
          ...
          <span
            data-hero-accent
            class="absolute -top-3 right-6 text-2xl text-rose-400 select-none"
            >✦</span
          >
          ...
          <svg
            data-hero-deco
            aria-hidden="true"
            class="absolute -top-2 -left-3 w-7 h-7 text-rose-300"
            viewBox="0 0 24 24"
            fill="none"
          >
            <path
          ...
          <svg
            data-hero-deco
            aria-hidden="true"
            class="absolute bottom-2 -right-2 w-6 h-6 text-rose-300"
            viewBox="0 0 24 24"
            fill="none"
          >
            <path
              d="M12 5 L19 19 L5 19 Z"
              stroke="currentColor"
              stroke-width="1.5"
          ...
          <span
            data-hero-deco
            class="absolute bottom-8 -left-2 block w-2.5 h-2.5 rounded-full bg-rose-400"
          ></span>
          <span
            data-hero-deco
            class="absolute top-10 -right-4 block w-1.5 h-1.5 rounded-full bg-rose-300"
          ></span>
        </div>
      </div>
    </div>
...
<style>
/* Illustrator行 */
.hero-cloak .hero-line:not(:has(.hero-char)),
