...
<template>
    <div ref="root" class="flex flex-col items-center my-12 mx-2">
        <div class="flex items-center gap-4 w-full max-w-5xl">
            <div data-fade class="flex-1 border-t-2 border-dashed border-gray-300 dark:border-neutral-700"></div>
            <div class="flex flex-col items-center">
                <span v-if="index" data-fade class="text-xs font-mono tracking-widest text-rose-400 dark:text-rose-400 mb-1">{{ index }}</span>
                <h2 data-title class="text-2xl font-bold tracking-tight px-2 overflow-hidden">{{ title }}</h2>
            </div>
            <div data-fade class="flex-1 border-t-2 border-dashed border-gray-300 dark:border-neutral-700"></div>
        </div>
        <svg data-fade viewBox="0 0 200 12" xmlns="http://www.w3.org/2000/svg" class="w-32 mt-2 text-rose-200 dark:text-rose-800">
